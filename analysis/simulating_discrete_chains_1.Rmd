...
3. For t = 1...T:   
      ...
      2. Generate $X_{t+1}$ using the inverse transform method according to the row obtained in step (1).
...
run.mc.sim <- function( P, num.iters = 50 ) {
  ...
  for(t in 2:num.iters) {
    ...
    # probability vector to simulate next state X_{n+1}
    p  <- P[states[t-1], ]
...
Our function returns a vector that contains the states of each of our simulated chains through time. Recall that our state space is $\{1,2,3\}$. Below, we  visualize how these chains evolve through time:
```{r}
...
Next we will do a similar, but larger, experiment with the size of our state space equal to 8.  Assume our probability transition matrix is:
$$P = \begin{bmatrix}
