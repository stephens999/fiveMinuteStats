...
Thus, for any decision rule $\hat{Y}$ we have that the risk is no better than $\hat{Y}_\text{opt}$:
$$r(\hat{Y}) \geq \int \biggl[ \int L(\hat{Y}_\text{opt}(X),Y) p(Y|X) \, dY \biggr] p(X) \, dX = r(\hat{Y}_\text{opt}).$$
