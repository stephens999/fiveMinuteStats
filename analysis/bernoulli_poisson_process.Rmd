...
For a Poisson process, arrivals may occur at any arbitrary time $t \geq 0$. One way to specify a Poisson process is by the counting process $N_t$ denoting the number of arrivals up to and including time $t$. For a Poisson process with rate paramters $\lambda$, we have that 
...
compare.density <- function(b.process, delta, inspect.at=10) {
  ...
  # plot
  plot(tt/sum(tt), main=paste("delta = ",round(delta,4), "trials per second"), xlab='number of arrivals after 10 seconds', ylab='density', xlim=c(0,15), ylim=range(c(tt/sum(tt), pois.density)))
