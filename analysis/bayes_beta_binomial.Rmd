...
This vignette illustrates how to perform Bayesian inference for a continuous parameter, specifically a binomial proportion. It introduces two key ideas: first the mechanics of how we actually calculate the posterior distribution; and second how we use this posterior distribution to do inference (e.g. interval estimates and point estimates).
...
- In simple cases you may find that the result is the density of a distribution you recognize. If so, you can often use known properties of that distribution to comptue quantities of interest. [Example](summarize_interpret_posterior.html).

- In cases where you do not recognize the posterior distribution, you may need
to use computational methods to compute quanties of interest.
