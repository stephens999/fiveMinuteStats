...
This vignette illustrates how to perform Bayesian inference for a binomial proportion. It also introduces the idea of conjugacy, or "conjugate prior" distributions.
...
However, in this case we are lucky: the posterior distribution is a nice distribution that we recognize, and this means we can do lots of calculations very easily: `R` has lots of built-in functions to deal with the Beta distribution, and many analytic properties have been derived (e.g. [Wikipedia](https://en.wikipedia.org/wiki/Beta_distribution).)
