...
This document assumes familiarity with the concepts of likelihoods, [likelihood ratios](http://stephens999.github.io/fiveMinuteStats/analysis/likelihood_ratio_simple_models.html), and hypothesis testing. 
...
As seen in the fiveMinuteStats on [likelihood ratios](http://stephens999.github.io/fiveMinuteStats/analysis/likelihood_ratio_simple_models.html), given the observed data $X_1\ldots,X_n$, we can measure the relative plausibility of $H_1$ to $H_0$ by the log-likelihood ratio:
...
Note that $\Theta_1$ in this case is the set of all $\lambda \neq \lambda_0$. In the numerator of the expression for $\Lambda_n$, we seek $\max_{\theta \in \Omega}f(X_1,\ldots,X_n|\theta)$. This is just the maximum likelihood estimate of $\lambda$ which we derived in [this note](http://mbonakda.github.io/fiveMinuteStats/analysis/asymptotic_normality_mle.html). The MLE is simply the sample average $\bar{X}$. The likelihood ratio is therefore:
