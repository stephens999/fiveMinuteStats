...
This document assumes basic familiarity with simulating Markov chains, as seen in this [note](simulating_discrete_chains_1.html).
...
P <- t(matrix(c( 0.7, 0.2, 0.1, 
                 0.4, 0.6,   0, 
                   0,   1,   0  ), nrow=3, ncol=3))
```
We will inspect the limiting probabilities and compare them to the stationary distribution. In [this note](stationary_distribution.html), we derived the stationary distribution for this example. Recall that the stationary distribution $\pi$ is the vector such that 
$$\pi = \pi P$$
