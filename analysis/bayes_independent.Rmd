...
Suppose we want to do inference for multiple parameters, and suppose that the data that are informative for each parameter are independent. Then provided the prior distributions on these parameters are independent, the posterior distributions are also independent. 
...
Furthermore, we know from [here](bayes_conjugate.html) that if the prior distribution on $q_j$ is a Beta distribution, say $q_j \sim \text{Beta}(a_j,b_j)$, then the posterior $p(q_j | n_j)$
